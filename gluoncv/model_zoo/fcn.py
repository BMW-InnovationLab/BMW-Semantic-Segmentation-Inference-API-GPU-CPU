...
class FCN(SegBaseModel):
    ...
    def __init__(self, nclass, backbone='resnet50', classes=1000,pretrainedbase="none",aux=True, ctx=cpu(), pretrained_base=True,
                 base_size=520, crop_size=480, **kwargs):
        super(FCN, self).__init__(nclass, aux, backbone,pretrainedbase=pretrainedbase, classes=classes,ctx=ctx, base_size=base_size,
...
def get_fcn(dataset='pascal_voc', backbone='resnet50', pretrained=False,
            root='~/.mxnet/models', pretrainedbase='none',classes=1000,ctx=cpu(0), pretrained_base=True, **kwargs):
    ...
    model = FCN(datasets[dataset].NUM_CLASS, backbone=backbone, pretrained_base=pretrained_base,
                ctx=ctx,pretrainedbase=pretrainedbase,classes=classes, **kwargs)
    model.classes = datasets[dataset].classes
    if pretrained:
        from .model_store import get_model_file
        model.load_parameters(get_model_file(
            'fcn_%s_%s'%(backbone, acronyms[dataset]), tag=pretrained, root=root), ignore_extra=True,ctx=ctx)
    return model
