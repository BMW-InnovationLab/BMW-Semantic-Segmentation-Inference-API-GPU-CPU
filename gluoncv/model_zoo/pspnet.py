...
class PSPNet(SegBaseModel):
    ...
    def __init__(self, nclass, backbone='resnet50', aux=True, ctx=cpu(), pretrained_base=True,
                 base_size=520,pretrainedbase='none' ,classes=1000,crop_size=480, **kwargs):
        super(PSPNet, self).__init__(nclass, aux, backbone,pretrainedbase=pretrainedbase,classes=classes, ctx=ctx, base_size=base_size,
...
def get_psp(dataset='pascal_voc', backbone='resnet50', pretrained=False,
    ...
    model = PSPNet(datasets[dataset].NUM_CLASS, backbone=backbone,
                   pretrained_base=pretrained_base,pretrainedbase=pretrainedbase,classes=classes, ctx=ctx, **kwargs)
