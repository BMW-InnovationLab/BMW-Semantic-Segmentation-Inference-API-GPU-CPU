...
class DeepLabV3(SegBaseModel):
    ...
    """
    def __init__(self, nclass,pretrainedbase='none',classes=1000,backbone='resnet50', aux=True, ctx=cpu(), pretrained_base=True,
        ...
        super(DeepLabV3, self).__init__(nclass, aux, backbone, ctx=ctx, base_size=base_size,
                                     crop_size=crop_size, classes=classes,pretrainedbase=pretrainedbase,pretrained_base=pretrained_base, **kwargs)
...
def get_deeplab(dataset='pascal_voc', backbone='resnet50', pretrained=False,
            root='~/.mxnet/models',pretrainedbase='none',classes=1000, ctx=cpu(0),oclasses=0, **kwargs):
    r"""DeepLabV3
    Parameters
    ----------
    ...
    if datasets[dataset].NUM_CLASS != 0:
        model = DeepLabV3(datasets[dataset].NUM_CLASS,pretrainedbase=pretrainedbase,classes=classes, backbone=backbone, ctx=ctx, **kwargs)
    else :
        model = DeepLabV3(oclasses, backbone=backbone,classes=classes, pretrainedbase=pretrainedbase,ctx=ctx, **kwargs)
    ...
    if pretrained:
        ...
        model.load_parameters(get_model_file('deeplab_%s_%s'%(backbone, acronyms[dataset]),
                                             tag=pretrained, root=root),ignore_extra=True, ctx=ctx)
    return model
