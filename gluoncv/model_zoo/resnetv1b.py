...
def resnet50_v1b(pretrained=False,pretrainedbase='none', classes=1000,root='~/.mxnet/models', ctx=cpu(0), **kwargs):
    ...
    use_global_stats : bool, default False
        ...
        optionally set to True if finetuning using ImageNet classification pretrained models.
    """
    model = ResNetV1b(BottleneckV1b, [3, 4, 6, 3], classes=int(classes),name_prefix='resnetv1b_', **kwargs)
    if pretrainedbase!="none":

        model.load_parameters(pretrainedbase,ignore_extra=True,allow_missing=True,ctx=ctx)
...
def resnet101_v1b(pretrained=False, pretrainedbase='none',classes=1000,root='~/.mxnet/models', ctx=cpu(0), **kwargs):
    ...
    use_global_stats : bool, default False
        ...
        optionally set to True if finetuning using ImageNet classification pretrained models.
    """
    model = ResNetV1b(BottleneckV1b, [3, 4, 23, 3], classes=int(classes),name_prefix='resnetv1b_', **kwargs)
    if pretrainedbase!="none":
        model.load_parameters(pretrainedbase,ignore_extra=True,allow_missing=True,ctx=ctx)
        print("LOADING CLASSIFIER PARAMETERS")
    elif pretrained:
...
def resnet50_v1s(pretrainedbase='none',classes=1000,pretrained=False, root='~/.mxnet/models', ctx=cpu(0), **kwargs):
    ...
    norm_layer : object
        ...
        Can be :class:`mxnet.gluon.nn.BatchNorm` or :class:`mxnet.gluon.contrib.nn.SyncBatchNorm`.
    """
    model = ResNetV1b(BottleneckV1b, [3, 4, 6, 3],classes=int(classes), deep_stem=True, stem_width=64,
...
def resnet101_v1s(pretrainedbase='none',classes=1000,pretrained=False, root='~/.mxnet/models', ctx=cpu(0), **kwargs):
    ...
    norm_layer : object
        ...
        Can be :class:`mxnet.gluon.nn.BatchNorm` or :class:`mxnet.gluon.contrib.nn.SyncBatchNorm`.
    """
    model = ResNetV1b(BottleneckV1b, [3, 4, 23, 3],classes=int(classes) ,deep_stem=True, stem_width=64,
                      name_prefix='resnetv1s_', **kwargs)
    if pretrainedbase!="none":
        model.load_parameters(pretrainedbase,ignore_extra=True,allow_missing=True,ctx=ctx)
        print("LOADING CLASSIFIER PARAMETERS")
